...
import serial
import time
from eye_blink import process_blinks
import f_detector
...
female = [
    ...
    "Glasses/glass17",
    "Glasses/glass14",
    "Glasses/sunglasses-kitty-02.png",
]
...
def get_customer_name(predicted_id):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT customer_name FROM customers WHERE customer_uid = ?", (predicted_id,)
    )
    result = cursor.fetchone()
    conn.close()
    if result:
...
def add_ok_sign_column():
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "ALTER TABLE customers ADD COLUMN ok_sign_detected INTEGER DEFAULT 0"
        )
        conn.commit()
...
def update_ok_sign_detected(predicted_id, ok_sign_detected):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE customers SET ok_sign_detected = ? WHERE customer_uid = ?",
        (ok_sign_detected, predicted_id),
    )
    conn.commit()
    conn.close()
...
def fetch_cart_details(customer_id):
    ...
    cursor.execute(
        "SELECT customer_name FROM customers WHERE customer_uid = ?", (customer_id,)
    )
    customer_name = cursor.fetchone()[0]

    cursor.execute(
        "SELECT item_name, item_count FROM cart WHERE customer_uid = ?", (customer_id,)
    )
    ...
    while True:
        ...
        for x, y, w, h in faces:
            ...
            sunglasses_height = int(
                sunglasses_width * sunglasses.shape[0] / sunglasses.shape[1]
            )

            # Resize the sunglasses image
            sunglasses_resized = cv2.resize(
                sunglasses, (sunglasses_width, sunglasses_height)
            )
            ...
            for c in range(0, 3):
                frame_roi[:, :, c] = (1.0 - sunglasses_mask) * frame_roi[
                    :, :, c
                ] + sunglasses_mask * sunglasses_resized[:, :, c]
        ...
        if ok_sign_detected and (current_time - last_ok_sign_time >= 3):
            ...
            if conf >= 45:
                ...
                ok_sign_count = add_item_to_cart(
                    id_, sunglasses_name
                )  # Add the sunglasses to the cart and get the count
